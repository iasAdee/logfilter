...
from color_rgb import page_12_layout
    ...
    for i, row in data.iterrows():
        ...
        if(pd.isna(val1)):
            ...
            if(val1 == "E2" or val1 == "E1"):
                if(val2<=1379000):
                    ls.append("green")
                elif(val2>1380000 and val2<= 2070000):
                    ls.append("orange")
                elif(val2 > 2070000):
                    ls.append("red")
            elif(val1 == "P3a" or val1 == "P3b"):
                if(val2<=120):
                    ls.append("green")
                elif(val2>= 120 and val2< 180):
                    ls.append("orange")
                elif(val2>=180):
                    ls.append("red")
            elif(val1 == "P5c"):
                if(val2<=289000):
                    ls.append("green")
                elif(val2>= 290000 and val2< 430000):
                    ls.append("orange")
                elif(val2>=430000):
                    ls.append("red")
            elif(val1 == "P6b"):
                ...
                if(val2<=19199):
                    ls.append("green")
                elif(val2>= 19200 and val2< 28800):
                    ls.append("orange")
                elif(val2>=28800):
                    ls.append("red")
            else:
...
@callback(
    ...
    Output('cc', 'data'),
     #Output('status', 'children'),
    [Input('upload-cc', 'contents')],
    [State('upload-cc', 'filename'),
     State('upload-cc', 'last_modified')]
)
def update_output(list_of_contents, list_of_names, list_of_dates):
...
@callback(
	#Output('output-data-upload', 'children'),
	Output('status_de31', 'children'),
	Output('results_rgb', 'children'),
	Output('results_r', 'children'),
...
def update_output(data):
	...
	if(len(df)>0):
		...
		orange = data[data["result"] == "orange"]
		green = data[data["result"] == "green"]
		...
		html.H3(f"Total Errors Lines In Table are: {counter}")

		])
		)

		def get_ls(data, color="lightcyan",size="500px"):
			ls = []
			ls.append(html.Div([
			dash_table.DataTable(
			    style_table={'height':size, 'overflowY': 'auto', 'width':'98%', 'margin-left':'4px'},
			    data=data.to_dict('records'),
			    columns=[{"name": i, "id": i} for i in data.columns],
		...
		ls = get_ls(data)     # Base color for main data
		lsr = get_ls(red, '#ff9c9c',size="350px")       # Light red
...
def process_200_images(image_bytes_list):
    ...
    --------------------------------------------
    - Extract exactly ONE numeric string printed on the round bottle bottom.
    - Length must be between 8 and 10 digits.
    - Do not predict number of image ending with 3 based on number of image ending with 2
	...
	if triggered_id == "process-btn":
		...
		if content is not None:
			...
			for prompt_parts in prompt_list:
			    while True: # Loop to keep trying the same prompt until success
			        try:
			            ...
			            # Check for the specific Quota/Rate Limit error
			            if "Quota exceeded" in error_message and "retry_delay" in error_message:
			                
			                # A robust way to extract the delay is complex, but often the message
			                # provides a clear number like 'seconds: 9'.
			                # For this example, we'll use the clear '9' from your previous message:
			                
			                delay_seconds = 10 # Safely wait a little more than the 9.26s indicated
			                print(f"Quota exceeded. Waiting for {delay_seconds} seconds before retrying...")
			                time.sleep(delay_seconds)
			                continue # Go back to the beginning of the while loop to retry the request
			            else:
			                # Handle other types of errors (e.g., authentication, invalid prompt)
...
app.layout = html.Div([
	...
	html.Div(id='page-11-content', style={'display': 'none'}, children=[
	    page_11_layout
	]),

	html.Div(id='page-12-content', style={'display': 'none'}, children=[
	    page_12_layout
	])
...
def display_page(pathname,id_, pass_):
    ...
    if(id_ == "log" and pass_ == "log"):#C3asar!
        ...
        return {'display': 'none'}, {'display': 'none'}, {'display': 'block'},{'display': 'none'},{'display': 'none'},{'display': 'none'},{'display': 'none'},{'display': 'none'},{'display': 'none'},{'display': 'none'},html.H6("Bitte ID und Passwort eintragen",style={"color":"black"})
    else:
