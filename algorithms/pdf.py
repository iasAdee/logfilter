...
def make_image_lists(doc):
    ...
    for page_number in range(len(doc)):
        ...
        if("fert artikel vom po:" in charge):
            artikle = charge[charge.index("fert artikel vom po:")+1]

        if("fert charge vom po:" in charge):
            charge_value = charge[charge.index("fert charge vom po:")+1]
        ...
        ref[text.split(":")[1].strip()] = [first_val, second_val,charge_value,artikle]
...
def parse_po_responses(responses):
    ...
    columns = [
        "FERT-Bez_Bild1", "BatchNO_Bild1", "Datum_Bild1", "ML_Bild1", "ArtikelNr_Bild1",
        "FERT-Bez_Bild2", "BatchNO_Bild2", "Datum_Bild2", "ML_Bild2", "ArtikelNr_Bild2", "Halb-Charge_Flasche1", "Halb-Charge_Flasche2"
    ]
    ...
    for po_num, sub_num, content, _ in po_blocks:
        ...
        if po_num != current_po:
            ...
            row = {"Auftrags-Nummer": current_po}
    ...
    df = pd.DataFrame(rows, columns=["Auftrags-Nummer"] + columns)
    return df
...
def calculate_pdf_scores(doc, selected_model,  api_key=""):
    ...
    print(ref)
    df_dict = pd.DataFrame.from_dict(ref, orient='index', columns=['Material', 'Menge', "charge_val", "artikle"])
    df_dict.index.name = 'Auftrags-Nummer'
    ...
    for i, prompt_parts in enumerate(prompt_list):
        ...
        print(f"Payload size: {sys.getsizeof(prompt_parts) / 1024**2:.2f} MB")
        success = False
        while not success:
            try:
    ...
    df_merged = data.merge(df_dict, on='Auftrags-Nummer', how='inner')
    ...
    df_merged['Halb-Charge_Flasche1'] = df_merged['Halb-Charge_Flasche1'].apply(extract_main_id)
    df_merged['Halb-Charge_Flasche2'] = df_merged['Halb-Charge_Flasche2'].apply(extract_main_id)
    ...
    def create_match_column(df):
        ...
        condition = (
            ...
            # ---------------------------
            # VALID / NON-EMPTY CHECKS
            # ---------------------------

            valid(df['FERT-Bez_Bild1']) &
            valid(df['FERT-Bez_Bild2']) &

            valid(df['ML_Bild1']) &
            valid(df['ML_Bild2']) &

            valid(df['BatchNO_Bild1']) &
            valid(df['BatchNO_Bild1']) &

            valid(df['Halb-Charge_Flasche1']) &
            valid(df['Halb-Charge_Flasche2']) &

            valid(df['Datum_Bild1']) &
            valid(df['Datum_Bild2']) &

            valid(df['Material']) &
            valid(df['charge_val']) &
            valid(df['artikle']) &

            valid(df['ArtikelNr_Bild1']) &
            valid(df['ArtikelNr_Bild1']) &
            ...
            (
                df['FERT-Bez_Bild1'].astype(str).str.strip() ==
                df['FERT-Bez_Bild2'].astype(str).str.strip()
            ) &

            (
                df['ML_Bild1'].astype(str).str.strip() ==
                df['ML_Bild2'].astype(str).str.strip()
            ) &

            (
                df['BatchNO_Bild1'].astype(str).str.strip() ==
                df['BatchNO_Bild1'].astype(str).str.strip() 
            ) &
            (
                df['BatchNO_Bild1'].astype(str).str.strip() ==
                df['charge_val'].astype(str).str.strip()
            ) &
            (
                df['Halb-Charge_Flasche1'].astype(str).str.strip() ==
                df['Halb-Charge_Flasche2'].astype(str).str.strip()
            ) &

            (
                df['Datum_Bild1'].astype(str).str.strip() ==
                df['Datum_Bild2'].astype(str).str.strip()
            ) &
            ...
            (
                df['ArtikelNr_Bild1'].astype(str).str.strip() ==
                df['ArtikelNr_Bild1'].astype(str).str.strip()
            ) & 
            (
                df['ArtikelNr_Bild1'].astype(str).str.strip() ==
                df['artikle'].astype(str).str.strip()
            )
        )

        df.loc[condition, 'Match'] = 'übereinstimmend'

        return df

    punct_cols = ['Halb-Charge_Flasche1', 'Halb-Charge_Flasche2']
